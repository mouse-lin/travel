...
module QuickSearch
   ...
   #搜索封装方法
     def quick_search type,limit = 10
       #一开是就赋值给product
       ...
       if params[:quick_search] || params[:quick_search_page] || params[:search]
        ...
        if type == "form"
          @results = Linefatuan.search(search_hash).paginate(:page => params[:quick_search_page],:per_page => limit).each do |l|
          ...
          limit =  limit
          count = Linefatuan.search(search_hash).count.to_f / limit.to_f
          count = count.ceil
        ...
        @results.uniq!
        @search_results.uniq!
        render_json @search_results,count if type == "ajax"
      end
    end   
    def render_json json = "operation success",count = 10
